...
function chainPromises(array, action) {
  const chainProm = array.reduce(async (chainAccumulator, currentPromise) => {
    try {
      const curRes = await currentPromise;
      const chainRes = await chainAccumulator;
      if (curRes && chainRes) {
        return action(chainRes, curRes);
      }
  ...
  return chainProm.then((r) => r).catch(() => 'ERROR');
}
